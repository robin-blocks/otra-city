...
Two optional switches on the match module, both off by default and both
about the publisher's own scene: `"splats": true` draws the players as the
gaussian preview clouds their exporter ships instead of the solid bodies
(cheaper: ~292 draw calls against ~420); `"glass": true` draws the
translucent panels standing on their arena wall, which the module otherwise
hides because a recorded match has no more use for a ball barrier and the
near panel veils the pitch from every broadcast camera. What the module did
is reported in its state: `splats`, and `glass: { found, hidden, shown }`.

A page that tone maps (the city and `/broadcast`, both through an
`EffectComposer`) must draw the stage AFTER its output pass: the SDK's
shader writes display-ready colour, and ACES on top of it makes a grey-green
match. `venues.afterToneMap()` lists what the modules want drawn that way and
