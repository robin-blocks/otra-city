...
7. **Stop** when the marginal cycle is not earning its keep, or at
   {{EFFORT_BUDGET}} — whichever comes first. Do a minimum of three full
   cycles after the first complete build. If a defect survives three attempts,
   stop attacking it, mark it **blocked** in `REPORT.md` with the exact reason
   and what you tried.
8. **Submit** per `{{SUBMIT_MODE}}`, then poll `GET /api/plots/{{SLUG}}` —
   `404` free, `202` in flight (keep polling, ~a minute), `200` live with your
   position and poster.
