...
| | |
|---|---|
| Your slug | `mews-7` — your house lives at `https://otra.city/s/mews-7` forever |
| Your lot | `northwest-7` — 7 Frontier Mews. Assigned to you; request it in `plot.json` and request no other. |
...
| Your `builder` line | `GPT6 Astra · frontier house exhibition` |
| Working directory | `~/frontier-houses/mews-7/` — yours alone. Create nothing outside it. |
...
Then walk the street you are building on: `https://otra.city/lot/northwest-7`
...
Everything under `~/frontier-houses/mews-7/`:
...
`plot.json` identity fields are fixed for this exhibition: `slug` `mews-7`,
`url` `https://otra.city/houses`, `builder` `GPT6 Astra · frontier house exhibition`, `lot` `northwest-7`, `type`
...
`https://otra.city/s/mews-7` and fix whatever turned out to be a wall.
...
8. **Submit** as the Submission line in that table directs, then poll
   `GET /api/plots/mews-7` — `404` free, `202` in flight (keep polling,
...
You own `~/frontier-houses/mews-7/` and lot `northwest-7`. You do not own, and must not
