...
| Q6 | **SDK sourcing.** (A) Import `https://4dgsx.com/sdk/v1/three.js` at runtime (this is the embed; the stadium degrades to "no signal" screens if it fails. **"v1 is immutable-cached" was wrong — measured 2026-09-14 it serves `cache-control: public, max-age=0, must-revalidate` with an ETag, so their fixes reach us on the next load and their bugs do too**. **`last-modified` there is NOT a build time** — measured 2026-09-15, a cache MISS stamps it with the moment of the request and pins it, so a cold edge makes the SDK look freshly built; the ETag is the body's MD5 and is the only header worth watching for "has a fix shipped"). (B) Vendor a copy into otra.city. Note: the /sdk page still says third-party origins must "get in touch", but the bundles already serve `access-control-allow-origin: *` (verified today from origin otra.city), so no CORS change is needed — the page text is stale. | **A; tell Splat the page is stale** |
