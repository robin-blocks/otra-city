...
| check | tool | evidence |
|---|---|---|
| plat current, ids unique, lots front their road, no overlaps, registry/manifest consistent, fence continuous along every road and from the spawn to every lot, a yard behind every lot and rows that back onto each other meet, no post in any spawn | `npm run map:check` | its PASS/FAIL table |
