...
export function createMediaSystem(camera) {
  ...
  function attach(container, gltfScene, media) {
    if (!media) return;
    if (media.audio) attachAudio(container, media.audio);
    if (media.screens) attachScreens(container, gltfScene, media.screens);
    if (media.feed) attachFeed(gltfScene, media.feed);
  }
