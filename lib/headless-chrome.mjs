...
// `sandbox: true` keeps Chrome's renderer sandbox on. The default is off
// because the poster path renders OUR OWN pages on a CI container that may be
// root, where the sandbox cannot start at all. Any path that points this
// browser at a page somebody else wrote must ask for the sandbox: the runner
// holds a token with write access to this repository, and an unsandboxed
// renderer parsing a stranger's site is the wrong place to find out that
// Chrome had a bug. Such a caller should treat a failed launch as "no
// screenshot", never as a reason to retry without it.
export async function launchChrome({ width = 1536, height = 864, timeoutMs = 30000, args = [], gpu = false, sandbox = false } = {}) {
  ...
  const child = spawn(bin, [
    ...
    // CI containers run as root, where the sandbox cannot start; the poster
    // path only ever loads our own pages. See the note on `sandbox` above.
    ...(sandbox ? [] : ['--no-sandbox']),
  ...
  return {
    // Raw DevTools access for the rare thing the helpers below do not cover
    // (the client check emulates a phone this way). Session-scoped.
    send: (method, params = {}, timeoutMs = 60000) => conn.send(method, params, sessionId, timeoutMs),
    evaluate,
