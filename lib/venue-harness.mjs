...
export async function openFixture({ venue, tier = null, fast = true, street = true, bundle = null, cam = null,
  width = 1536, height = 864, gpu = false, timeoutMs = 60000, onEvaluate = null } = {}) {
  ...
  // `opts` passes through so a caller can raise the evaluate budget for one
  // call — see `coplanar` below, and the SDK mount in venue-check, both of
  // which are slow for reasons that have nothing to do with the fixture.
  // Optional diagnostics only: preserve every expression, ordering and timeout.
  // The start event is emitted BEFORE dispatch, so even a stuck CDP call is named.
  let evaluation = 0;
  const ev = async (expr, opts) => {
    const id = ++evaluation, started = Date.now();
    const emit = (event) => { try { onEvaluate?.({ id, expression: expr.slice(0, 240), ...event }); } catch { /* diagnostics cannot change the gate */ } };
    emit({ phase: 'start', at: new Date(started).toISOString(), timeoutMs: opts?.timeoutMs ?? 60000 });
    try {
      const value = await chrome.evaluate(expr, opts);
      emit({ phase: 'end', elapsedMs: Date.now() - started });
      return value;
    } catch (error) {
      emit({ phase: 'error', elapsedMs: Date.now() - started, error: String(error?.message || error) });
      throw error;
    }
  };
  const api = {
    ...
    // its own timeout: thousands of rays per camera is CPU work that has
    // nothing to do with how fast the runner's renderer is, and it must not
    // be the call that trips the fixture's default evaluate budget
    coplanar: (opts = {}) => chrome.evaluate(`JSON.stringify(window.__venue.coplanar(${JSON.stringify(opts)}))`, { timeoutMs: 180000 }).then(JSON.parse),
    walkability: (opts = {}) => ev(`JSON.stringify(window.__venue.walkability(${JSON.stringify(opts)}))`).then(JSON.parse),
    spawnClearance: (opts = {}) => ev(`JSON.stringify(window.__venue.spawnClearance(${JSON.stringify(opts)}))`).then(JSON.parse),
    routeFromCity: (opts = {}) => ev(`JSON.stringify(window.__venue.routeFromCity(${JSON.stringify(opts)}))`).then(JSON.parse),
