...
/**
 * Does this hostname resolve to public addresses only?
 *
 * For anything that cannot use `fetchAsset` — a browser we are about to point
 * at a page, say — this is the same rule `guardedLookup` applies, run ahead of
 * time. It is strictly weaker: the caller resolves, then something else
 * connects, so a name that changes answers between the two calls is not
 * covered. Use `fetchAsset` whenever the thing being fetched is a file.
 */
export async function resolvesPublicly(hostname) {
  ...
  return new Promise((resolve) => {
    dnsLookup(literal, { all: true, verbatim: true }, (err, addresses) => {
      if (err || !addresses?.length) return resolve(false);
      resolve(!addresses.some((a) => isPrivateAddress(a.address)));
    });
  });
}

/**
 * Fetch one file, with the address guard, a byte cap and a redirect limit.
 *
 * With `withUrl`, resolves to `{ buffer, url }` where `url` is the address
 * actually served — a caller that has to resolve relative links against the
 * page (og:image, say) needs the post-redirect url, not the one it asked for.
 */
export async function fetchAsset(rawUrl, { maxBytes, label = 'file', timeoutMs = 20000, maxRedirects = 3, withUrl = false } = {}) {
  let url;
  try {
    url = new URL(rawUrl);
  } catch {
  ...
  for (let hop = 0; ; hop++) {
    ...
    return withUrl ? { buffer, url: url.href } : buffer;
  }
}
