...
import { CATEGORY_IDS, DEFAULT_CATEGORY } from '../public/js/categories.mjs';
...
export function validateIdentity(plot) {
  ...
  check('tagline', typeof plot.tagline === 'string' && plot.tagline.length <= 80,
    typeof plot.tagline === 'string' ? `${plot.tagline.length}/80 chars`
      : 'missing — send a tagline (<= 80 chars), or a description and the city derives one');
  // The directory fields, in PromptFrenzy's AI-directory shape so one payload
  // lists a project in both. Category is what places a plot: every road that
  // takes listings serves one or more categories, and an unset one is `other`.
  const cat = plot.category;
  const catOk = cat === undefined || CATEGORY_IDS.includes(cat);
  check('category', catOk, cat === undefined
    ? `none — placed as "${DEFAULT_CATEGORY}"; set one of ${CATEGORY_IDS.join(', ')} to land beside your kind`
    : catOk ? `"${cat}"` : `"${cat}" is not a category — one of ${CATEGORY_IDS.join(', ')}`);
  const desc = plot.description;
  check('description', desc === undefined || (typeof desc === 'string' && desc.length <= 200),
    desc === undefined ? 'none (optional, <= 200 chars — shown on your listing page)'
      : typeof desc === 'string' ? `${desc.length}/200 chars` : 'must be a string');
  const PRICING = ['free', 'freemium', 'paid', 'subscription'];
  check('pricing', plot.pricing === undefined || PRICING.includes(plot.pricing),
    plot.pricing === undefined ? 'none (optional)' : PRICING.includes(plot.pricing) ? `"${plot.pricing}"`
      : `"${plot.pricing}" — one of ${PRICING.join(', ')}`);
  const tags = plot.tags;
  const tagsOk = tags === undefined || (Array.isArray(tags) && tags.length <= 5 &&
    tags.every((t) => typeof t === 'string' && /^[a-z0-9][a-z0-9-]{0,23}$/.test(t)));
  check('tags', tagsOk, tags === undefined ? 'none (optional, up to 5, lowercase-hyphen)'
    : tagsOk ? tags.join(', ') : 'up to 5 tags, each lowercase letters/digits/hyphens, <= 24 chars');
  // What a VISITOR may do with the glb and media. The licence the CITY needs is
  // granted by submitting (docs/submission.md, plot-spec.json `terms`) and is not
  // this field; this one is the submitter's own terms for everyone else, stored
  // and displayed verbatim. Never verified and never enforced — declaring
  // CC-BY-4.0 changes nothing the city does — so the only check is that it is a
  // short single-line string, because it goes on a page.
  const lic = plot.license;
  const licOk = lic === undefined ||
    (typeof lic === 'string' && lic.length > 0 && lic.length <= 64 && !/[\n\r]/.test(lic));
  check('license', licOk, lic === undefined
    ? 'none (optional) — your files stay all rights reserved'
    : licOk ? `"${lic}" (shown on your listing page, not enforced)`
      : 'one line, <= 64 chars — an SPDX id (CC-BY-4.0, MIT, CC0-1.0) or a short phrase');
  ...
  // optional: which lot the plot asks for. Whether that lot exists and is
  // free is the submit API's and CI's business (they hold the map and the
  // registry); this is only the shape, so a typo fails at the same table.
  const lotOk = plot.lot === undefined || (typeof plot.lot === 'string' && /^[a-z][a-z0-9-]*-\d+$/.test(plot.lot));
  check('lot id', lotOk, plot.lot === undefined ? 'none — a listing is placed on the road for its category (the lot line below says where)'
    : lotOk ? `"${plot.lot}" requested` : `"${plot.lot}" is not a lot id (like boulevard-14 — see GET /api/plots vacant[])`);
...
// ---------------------------------------------------------------- media probes
//
// The published caps include an audio duration (90 s) and a screen resolution
// (1280x720). Nothing enforced either of them: the API checked extensions and
// byte sizes, CI checked neither, and submission.md listed "durations,
// resolution" among the mechanical gates. These read them.
//
// mp4 and m4a are both ISO base media files, so the numbers are a few box
// headers away and need no ffmpeg — which matters, because this has to run
// inside the submit function as well as in CI. mp3 and ogg are not ISO BMFF
// and stay size-checked only; that is stated in the check's own detail rather
// than left for a submitter to discover.
//
// AN UNREADABLE FILE IS NOT A REJECTION. A probe that cannot parse a file
// returns null and the check passes with "not probed": the browser is the real
// arbiter of playability, and a parser bug here must never be the thing that
// stops an honest plot from landing.
function* isoBoxes(buf, start, end) {
  let off = start;
  while (off + 8 <= end) {
...
const findBox = (buf, box, type) => {
  for (const b of isoBoxes(buf, box.start, box.end)) if (b.type === type) return b;
  return null;
};

// { duration_s, width, height, tracks: { video, audio } } — or null if this is
// not an ISO base media file we can read.
export function probeIsoBmff(buf) {
  if (!Buffer.isBuffer(buf) || buf.length < 16) return null;
  const root = { start: 0, end: buf.length };
  if (!findBox(buf, root, 'ftyp')) return null;
  const moov = findBox(buf, root, 'moov');
  if (!moov) return null;

  let duration_s = null;
  const mvhd = findBox(buf, moov, 'mvhd');
  if (mvhd) {
    const v = buf[mvhd.start];
    const [tsOff, durOff] = v === 1 ? [20, 24] : [12, 16];
    if (mvhd.start + durOff + (v === 1 ? 8 : 4) <= mvhd.end) {
      const timescale = buf.readUInt32BE(mvhd.start + tsOff);
      const dur = v === 1
        ? Number(buf.readBigUInt64BE(mvhd.start + durOff))
        : buf.readUInt32BE(mvhd.start + durOff);
      if (timescale > 0 && dur > 0 && dur !== 0xffffffff) duration_s = dur / timescale;
    }
  }
  ...
  for (const trak of isoBoxes(buf, moov.start, moov.end)) {
    if (trak.type !== 'trak') continue;
    const mdia = findBox(buf, trak, 'mdia');
    const hdlr = mdia && findBox(buf, mdia, 'hdlr');
    const kind = hdlr && hdlr.start + 12 <= hdlr.end
      ? buf.toString('latin1', hdlr.start + 8, hdlr.start + 12) : '';
    if (kind === 'vide') tracks.video += 1;
    if (kind === 'soun') tracks.audio += 1;
    const tkhd = findBox(buf, trak, 'tkhd');
    // width/height are the last 8 bytes of tkhd in both versions, 16.16 fixed
    if (kind === 'vide' && tkhd && tkhd.end - tkhd.start >= 8) {
      width = Math.max(width, Math.round(buf.readUInt32BE(tkhd.end - 8) / 65536));
      height = Math.max(height, Math.round(buf.readUInt32BE(tkhd.end - 4) / 65536));
    }
  }
  return { duration_s, width, height, tracks };
}

// Duration/resolution checks for a whole bundle: { "name.ext": Buffer }.
// Only files the manifest actually binds are judged against a role's caps.
export function probeMediaFiles(plot, files) {
  const M = SPEC.media;
  const checks = [];
  const base = (f) => (f || '').split('/').pop();
  const m = plot?.media || {};
  const audioName = base(m.audio?.file);
  const screenNames = new Set((m.screens || []).map((s) => base(s.file)));
  const [maxW, maxH] = M.screens.max_resolution;
  const maxAudio = M.ambient_audio.max_duration_s;

  for (const [name, buf] of Object.entries(files || {})) {
    const ext = name.split('.').pop().toLowerCase();
    const isAudio = name === audioName;
    const isScreen = screenNames.has(name);
    if (!isAudio && !isScreen) continue;                 // pictures/feed: nothing timed to check

    if (isAudio && ext !== 'm4a') {
      checks.push({ name: `probe ${name}`, ok: true,
        detail: `.${ext} is not an ISO container — size-checked only, duration not probed (cap is ${maxAudio} s; use m4a to have it verified)` });
      continue;
    }
    const info = probeIsoBmff(buf);
    if (!info) {
      checks.push({ name: `probe ${name}`, ok: true,
        detail: 'not probed — could not read the container (validated on size and extension only)' });
      continue;
    }
    const secs = info.duration_s;
    if (isAudio) {
      const ok = secs === null || secs <= maxAudio + 0.5;
      checks.push({ name: `probe ${name}`, ok,
        detail: secs === null ? 'duration not readable; size and format OK'
          : ok ? `${secs.toFixed(1)} s of ${maxAudio} s`
            : `${secs.toFixed(1)} s — over the ${maxAudio} s ambient-audio cap` });
      continue;
    }
    const fits = !info.width || !info.height || (info.width <= maxW && info.height <= maxH);
    const res = info.width && info.height ? `${info.width}x${info.height}` : 'resolution not readable';
    checks.push({ name: `probe ${name}`, ok: fits,
      detail: fits
        ? `${res}${secs === null ? '' : `, ${secs.toFixed(1)} s`} (max ${maxW}x${maxH})` +
          (info.tracks.audio ? ' — carries an audio track; screens always play silent' : '')
        : `${res} — over the ${maxW}x${maxH} screen cap` });
  }
  return { checks, ok: checks.every((c) => c.ok) };
}
